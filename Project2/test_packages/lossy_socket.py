...
class LossyUDP(socket):
    ...
    def sendto(self, message: bytes, dst: Tuple[str, int]):
        ...
        sleep(0.01)
        if random.random() < self.sim.loss_rate and not self.sim.forced_reliable():
            # drop the packet
            # print("outgoing UDP packet was dropped by the simulator.")
            pass
        else:
            if not self.sim.forced_reliable():
                ...
                for bit_to_flip in range(len(message) * 8):
                    ...
                    if random.random() < self.sim.corruption_rate / (len(message) * 8):
                        bits_flipped += 1
                        # corrupt the packet
            ...
            Timer(0 if self.sim.forced_reliable() else random.random() * self.sim.max_delivery_delay,
